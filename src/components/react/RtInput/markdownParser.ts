...
export type TagRenderer = Record<SupportedTag, (text: string) => ReactNode>;

export const parse = (input: string, tagRenderer: TagRenderer): ReactNode[] => {
  ...
  while (i < input.length) {
    ...
    if (range && isComplete(range)) {
      nodes[nodeIdx] = tagRenderer[range.name](nodes[nodeIdx] as string);
      range = undefined;
      nodeIdx++;
    }
    i++;
